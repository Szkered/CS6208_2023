...
bool Parameters::arrange() {
  ...
  if (mixing_parameter == unlikely) {
    ...
    cerr << "\n***********************\nERROR:\t mixing parameter unspecified"
         << endl;
    return false;
  }
  ...
  if (overlapping_nodes < 0 || overlap_membership < 0) {
    ...
    cerr << "\n***********************\nERROR:\tsome positive parameters are "
            "negative"
         << endl;
  ...
  if (num_nodes <= 0 || average_k <= 0 || max_degree <= 0 ||
      mixing_parameter < 0 || (nmax <= 0 && nmax != unlikely) ||
      (nmin <= 0 && nmin != unlikely)) {
  ...
  if (mixing_parameter > 1) {
    ...
    cerr << "\n***********************\nERROR:\tmixing parameter > 1 (must be "
            "between 0 and 1)"
         << endl;
  ...
  if (excess && defect) {
    ...
    cerr << "\n***********************\nERROR:\tboth options -inf and -sup "
  ...
  cout << "number of memberships of the overlapping nodes:\t"
       << overlap_membership << endl;
  if (clustering_coeff != unlikely)
    cout << "Average clustering coefficient: " << clustering_coeff << endl;

  if (fixed_range) {
    cout << "community size range set equal to [" << nmin << " , " << nmax
         << "]" << endl;

    if (nmin > nmax) {
      cerr << "\n***********************\nERROR: INVERTED COMMUNITY SIZE BOUNDS"
           << endl;
      return false;
    }

    if (nmax > num_nodes) {
      cerr << "\n***********************\nERROR: maxc BIGGER THAN THE NUMBER "
              "OF NODES"
           << endl;
      return false;
    }
...
bool Parameters::set(string &flag, string &num) {
  ...
  if (flag == command_flags[0]) {
    ...
    if (fabs(err - int(err)) > 1e-8) {
      ...
      cerr << "\n***********************\nERROR: number of nodes must be an "
              "integer"
           << endl;
      return false;
    }
    ...
    if (fabs(err - int(err)) > 1e-8) {
      ...
      cerr << "\n***********************\nERROR: the minumum community size "
              "must be an integer"
           << endl;
      return false;
    }
    ...
    if (fabs(err - int(err)) > 1e-8) {
      ...
      cerr << "\n***********************\nERROR: the maximum community size "
              "must be an integer"
           << endl;
      return false;
    }
    ...
    if (fabs(err - int(err)) > 1e-8) {
      ...
      cerr << "\n***********************\nERROR: the number of overlapping "
    ...
    if (fabs(err - int(err)) > 1e-8) {
      ...
      cerr << "\n***********************\nERROR: the number of membership of "
              "the overlapping nodes must be an integer"
           << endl;
      return false;
    }
    ...
    cerr << "\n***********************\nERROR while reading parameters: "
         << flag << " is an unknown option" << endl;
    return false;
  }
...
void statement() {
  ...
  cout << "----------------------\n" << endl;
  cout << "It is also possible to set the parameters writing flags and "
          "relative numbers in a file. To specify the file, use the option:"
       << endl;
  cout << "-f\t[filename]" << endl;
  cout << "You can set the parameters both writing some of them in the file, "
          "and using flags from the command line for others."
       << endl
       << endl;
  cout << "-N, -k, -maxk, -mu have to be specified. For the others, the "
          "program can use default values:"
       << endl;
  cout << "t1=2, t2=1, on=0, om=0, minc and maxc will be chosen close to the "
          "degree sequence extremes."
       << endl;
  cout << "If you don't specify -C the rewiring process for raising the "
          "average clustering coefficient will not be performed"
       << endl;
  ...
  cout << "-rand" << endl;
  cout << "Using this option will set mu=0, and minc=maxc=N, i.e. there will "
          "be one only community."
       << endl;

  cout << "Use option -sup (-inf) if you want to produce a benchmark whose "
          "distribution of the ratio of external degree/total degree ";
  cout << "is superiorly (inferiorly) bounded by the mixing parameter." << endl;

  cout << "\n-------------------- Examples ---------------------------\n"
       << endl;
  cout << "Example1:" << endl;
  cout << "./benchmark -N 1000 -k 15 -maxk 50 -mu 0.1 -minc 20 -maxc 50 -C 0.7"
       << endl;
  cout << "Example2:" << endl;
