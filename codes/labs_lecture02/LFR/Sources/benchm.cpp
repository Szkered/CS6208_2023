 ...
 *	Modified on 28/05/09 * Collaborators: Santo Fortunato
 ** Location: ISI foundation, Turin, Italy * Project: Benchmarking community
 *detection programs                           *
 *                                                                               *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 **
...
bool they_are_mate(int a, int b, const deque<deque<int>> &member_list) {
  ...
  for (int i = 0; i < member_list[a].size(); i++) {
    ...
    if (binary_search(member_list[b].begin(), member_list[b].end(),
                      member_list[a][i]))
      return true;
  }
...
double average_degree(const double &dmax, const double &dmin,
                      const double &gamma) {

  return (1. / (integral(gamma, dmax) - integral(gamma, dmin))) *
         (integral(gamma + 1, dmax) - integral(gamma + 1, dmin));
}

// bisection method to find the inferior limit, in order to have the expected
// average degree
double solve_dmin(const double &dmax, const double &dmed, const double &gamma) {
  ...
  if ((average_k1 - dmed > 0) || (average_k2 - dmed < 0)) {
    ...
    cerr << "\n***********************\nERROR: the average degree is out of "
            "range:";

    if (average_k1 - dmed > 0) {
      cerr << "\nyou should increase the average degree (bigger than "
           << average_k1 << ")" << endl;
      cerr << "(or decrease the maximum degree...)" << endl;
    }

    if (average_k2 - dmed < 0) {
      cerr << "\nyou should decrease the average degree (smaller than "
           << average_k2 << ")" << endl;
...
int build_bipartite_network(deque<deque<int>> &member_matrix,
                            const deque<int> &member_numbers,
                            const deque<int> &num_seq) {

  // this function builds a bipartite network with num_seq and member_numbers
  // which are the degree sequences. in member matrix links of the communities
  // are stored this means member_matrix has num_seq.size() rows and each row
  // has num_seq[i] elements
  ...
  */
  ...
  while (itlast != degree_node_in.begin()) {
    ...
    for (int i = 0; i < erasenda.size(); i++) {
      ...
      if (erasenda[i]->first > 1)
        degree_node_out.insert(
            make_pair(erasenda[i]->first - 1, erasenda[i]->second));
    ...
    for (int node_a = 0; node_a < num_seq.size(); node_a++)
      for (int krm = 0; krm < en_out[node_a].size(); krm++) {
        ...
        if (en_out[node_a].find(random_mate) == en_out[node_a].end()) {
          ...
          for (set<int>::iterator it_est = en_out[node_a].begin();
               it_est != en_out[node_a].end(); it_est++)
            external_nodes.push_back(*it_est);

          int old_node = external_nodes[irand(external_nodes.size() - 1)];

          deque<int> not_common;
          for (set<int>::iterator it_est = en_in[random_mate].begin();
               it_est != en_in[random_mate].end(); it_est++)
  ...
  for (int i = 0; i < en_out.size(); i++) {
    ...
    for (set<int>::iterator its = en_out[i].begin(); its != en_out[i].end();
         its++)
...
int internal_degree_and_membership(
    double mixing_parameter, int overlapping_nodes, int max_mem_num,
    int num_nodes, deque<deque<int>> &member_matrix, bool excess, bool defect,
    deque<int> &degree_seq, deque<int> &num_seq,
    deque<int> &internal_degree_seq, bool fixed_range, int nmin, int nmax,
    double tau2) {

  if (num_nodes < overlapping_nodes) {

    cerr << "\n***********************\nERROR: there are more overlapping "
            "nodes than nodes in the whole network! Please, decrease the "
            "former ones or increase the latter ones"
         << endl;
    return -1;
  }
  ...
  for (int i = 0; i < degree_seq.size(); i++) {
    ...
    if (excess) {
      ...
      while ((double(int_interno) / degree_seq[i] < (1 - mixing_parameter)) &&
             (int_interno < degree_seq[i]))
        int_interno++;
    }
  ...
  if (num_seq.empty()) {
    ...
    if (!fixed_range && (max_degree_actual + 1) > nmin) {
      ...
      _num_ = max_degree_actual +
              1; // this helps the assignment of the memberships (it assures
                 // that at least one module is big enough to host each node)
      num_seq.push_back(max_degree_actual + 1);
    }

    while (true) {

      int nn = lower_bound(cumulative.begin(), cumulative.end(), ran4()) -
               cumulative.begin() + nmin;
    ...
    num_seq[min_element(num_seq.begin(), num_seq.end()) - num_seq.begin()] +=
        num_nodes + overlapping_nodes * (max_mem_num - 1) - _num_;
  }
  ...
  for (int i=degree_seq.size() - overlapping_nodes; i<degree_seq.size(); i++) {
          ...
          while(members.size()<max_mem_num) {
                  ...
                  if(hh>3*num_nodes) {
                          cerr<<"it seems that the overlapping nodes need more
  communities that those I provided. Please increase the number of communities
  or decrease the number of overlapping nodes"<<endl; return -1;
                  }

          }


          for (set<int>::iterator its=members.begin(); its!=members.end();
  its++) member_matrix[*its].push_back(i);
  ...
  */
  ...
  if (build_bipartite_network(member_matrix, member_numbers, num_seq) == -1) {
    ...
    cerr << "it seems that the overlapping nodes need more communities that "
            "those I provided. Please increase the number of communities or "
            "decrease the number of overlapping nodes"
         << endl;
    return -1;
  }
      ...
      if (kr == 3 * num_nodes) {
        ...
        if (change_community_size(num_seq) == -1) {
          ...
          cerr << "\n***********************\nERROR: this program needs more "
                  "than one community to work fine"
               << endl;
          return -1;
        }

        cout << "it took too long to decide the memberships; I will try to "
                "change the community sizes"
             << endl;
        ...
        return (internal_degree_and_membership(
            mixing_parameter, overlapping_nodes, max_mem_num, num_nodes,
            member_matrix, excess, defect, degree_seq, num_seq,
            internal_degree_seq, fixed_range, nmin, nmax, tau2));
      }
    }
...
/*
int check_link_list(const deque<deque<int> > & link_list, const deque<int> &
degree_seq) {
...
int build_subgraph(deque<set<int>> &E, const deque<int> &nodes,
  ...
  */
  ...
  if (degrees.size() < 3) {
    ...
    cerr
        << "it seems that some communities should have only 2 nodes! This does "
           "not make much sense (in my opinion) Please change some parameters!"
        << endl;
    return -1;
  }

  // this function is to build a network with the labels stored in nodes and the
  // degree seq in degrees (correspondence is based on the vectorial index) the
  // only complication is that you don't want the nodes to have neighbors they
  // already have
    ...
    for (int node_a = 0; node_a < degrees.size(); node_a++)
      for (int krm = 0; krm < en[node_a].size(); krm++) {
        ...
        if (en[node_a].insert(random_mate).second) {
          ...
          deque<int> out_nodes;
          for (set<int>::iterator it_est = en[node_a].begin();
               it_est != en[node_a].end(); it_est++)
          ...
          deque<int> not_common;
          for (set<int>::iterator it_est = en[random_mate].begin();
               it_est != en[random_mate].end(); it_est++)
            if ((old_node != (*it_est)) &&
                (en[old_node].find(*it_est) == en[old_node].end()))
              not_common.push_back(*it_est);
  ...
  // now I try to insert the new links into the already done network. If some
  // multiple links come out, I try to rewire them
    ...
    for (set<int>::iterator its = en[i].begin(); its != en[i].end(); its++)
      if (i < *its) {
        ...
        bool already =
            !(E[nodes[i]]
                  .insert(nodes[*its])
                  .second); // true is the insertion didn't take place
        if (already)
  ...
  for (int i = 0; i < multiple_edge.size(); i++) {
    ...
    // now, I'll try to rewire this multiple link among the nodes stored in
    // nodes.
    int stopper_ml = 0;
    ...
    while (true) {
      ...
      if (E[a].find(random_mate) == E[a].end()) {
        ...
        deque<int> not_common;
        for (set<int>::iterator it_est = E[random_mate].begin();
             it_est != E[random_mate].end(); it_est++)
          if ((b != (*it_est)) && (E[b].find(*it_est) == E[b].end()) &&
              (binary_search(nodes.begin(), nodes.end(), *it_est)))
            not_common.push_back(*it_est);
      ...
      if (stopper_ml == 2 * E.size()) {
        ...
        cout << "sorry, I need to change the degree distribution a little bit "
                "(one less link)"
             << endl;
        break;
      }
...
int build_subgraphs(deque<set<int>> &E, const deque<deque<int>> &member_matrix,
                    deque<deque<int>> &member_list,
                    deque<deque<int>> &link_list,
                    const deque<int> &internal_degree_seq,
                    const deque<int> &degree_seq, const bool excess,
                    const bool defect) {
  ...
  for (int i = 0; i < member_list.size(); i++) {
    ...
    for (int j = 0; j < member_list[i].size(); j++) {
      ...
      compute_internal_degree_per_node(internal_degree_seq[i],
                                       member_list[i].size(), liin);
  ...
  // now there is the check for the even node (it means that the internal degree
  // of each group has to be even and we want to assure that, otherwise the
  // degree_seq has to change) ----------------------------

  // ------------------------ this is done to check if the sum of the internal
  // degree is an even number. if not, the program will change it in such a way
  // to assure that.
  ...
  for (int i = 0; i < member_matrix.size(); i++) {
    ...
    for (int j = 0; j < member_matrix[i].size(); j++) {
      ...
      int right_index = lower_bound(member_list[member_matrix[i][j]].begin(),
                                    member_list[member_matrix[i][j]].end(), i) -
                        member_list[member_matrix[i][j]].begin();
    ...
    if (internal_cluster % 2 != 0) {
      ...
      if (default_flag) {
        ...
        // if this does not work in a reasonable time the degree sequence will
        // be changed

        for (int j = 0; j < member_matrix[i].size(); j++) {

          int random_mate =
              member_matrix[i][irand(member_matrix[i].size() - 1)];

          int right_index = lower_bound(member_list[random_mate].begin(),
                                        member_list[random_mate].end(), i) -
                            member_list[random_mate].begin();

          if ((link_list[random_mate][right_index] <
               member_matrix[i].size() - 1) &&
              (link_list[random_mate][link_list[random_mate].size() - 1] > 0)) {
...
int connect_all_the_parts(deque<set<int>> &E,
  ...
  // now there is a rewiring process to avoid "mate nodes" (nodes with al least
  // one membership in common) to link each other
      ...
      for (set<int>::iterator its = en[a].begin(); its != en[a].end(); its++)
        if (they_are_mate(a, *its, member_list)) {
          ...
          while (true) {
            ...
            if (!(they_are_mate(a, random_mate, member_list)) &&
                (en[a].find(random_mate) == en[a].end())) {
          ...
          break; // this break is done because if you erased some link you have
                 // to stop this loop (en[i] changed)
        }
...
int internal_kin(deque<set<int>> &E, const deque<deque<int>> &member_list,
                 int i) {
...
int internal_kin_only_one(
    set<int> &E,
    const deque<int>
        &member_matrix_j) { // return the overlap between E and member_matrix_j
...
int erase_links(deque<set<int>> &E, const deque<deque<int>> &member_list,
  ...
  if (excess) {
    ...
    for (int i = 0; i < num_nodes; i++) {
      ...
      while ((E[i].size() > 1) &&
             double(internal_kin(E, member_list, i)) / E[i].size() <
                 1 - mixing_parameter) {

        //---------------------------------------------------------------------------------

        cout << "degree sequence changed to respect the option -sup ... "
             << ++eras_add_times << endl;

        deque<int> deqar;
        for (set<int>::iterator it_est = E[i].begin(); it_est != E[i].end();
             it_est++)
        ...
        if (deqar.size() == E[i].size()) { // this shouldn't happen...
          ...
          cerr << "sorry, something went wrong: there is a node which does not "
                  "respect the constraints. (option -sup)"
               << endl;
          return -1;
        }
  ...
  if (defect) {
    ...
    for (int i = 0; i < num_nodes; i++)
      while ((E[i].size() < E.size()) &&
             double(internal_kin(E, member_list, i)) / E[i].size() >
                 1 - mixing_parameter) {

        //---------------------------------------------------------------------------------

        cout << "degree sequence changed to respect the option -inf ... "
             << ++eras_add_times << endl;
        ...
        while (((they_are_mate(i, random_mate, member_list)) ||
                E[i].find(random_mate) != E[i].end()) &&
               (stopper_ < stopper_here)) {
        ...
        if (stopper_ == stopper_here) { // this shouldn't happen...
          ...
          cerr << "sorry, something went wrong: there is a node which does not "
                  "respect the constraints. (option -inf)"
               << endl;
          return -1;
        }
...
int print_network(deque<set<int>> &E, const deque<deque<int>> &member_list,
                  const deque<deque<int>> &member_matrix, deque<int> &num_seq) {
  ...
  for (int i = 0; i < member_matrix.size(); i++) {
    ...
    for (int j = 0; j < member_matrix[i].size(); j++) {
      ...
      double kinj = double(
          internal_kin_only_one(E[member_matrix[i][j]], member_matrix[i]));
      media_int += kinj;
      media_est += E[member_matrix[i][j]].size() -
                   double(internal_kin_only_one(E[member_matrix[i][j]],
                                                member_matrix[i]));
    }

    double pair_num = (member_matrix[i].size() * (member_matrix[i].size() - 1));
    double pair_num_e =
        ((num_nodes - member_matrix[i].size()) * (member_matrix[i].size()));
  ...
  cout << "network of " << num_nodes << " vertices and " << edges / 2
       << " edges"
       << ";\t average degree = " << double(edges) / num_nodes << endl;
  cout << "\naverage mixing parameter: " << average_func(double_mixing)
       << " +/- " << sqrt(variance_func(double_mixing)) << endl;
  ...
  statout << "degree distribution (probability density function of the degree "
             "in logarithmic bins) "
          << endl;
...
int benchmark(bool excess, bool defect, int num_nodes, double average_k,
              int max_degree, double tau, double tau2, double mixing_parameter,
              int overlapping_nodes, int overlap_membership, int nmin, int nmax,
              bool fixed_range, double ca) {
  ...
  if (!fixed_range) {
    ...
    cout << "-----------------------------------------------------------"
         << endl;
    cout << "community size range automatically set equal to [" << nmin << " , "
         << nmax << "]" << endl;
  }
  ...
  if (deque_int_sum(degree_seq) % 2 != 0)
    degree_seq[max_element(degree_seq.begin(), degree_seq.end()) -
               degree_seq.begin()]--;
  ...
  if (internal_degree_and_membership(
          mixing_parameter, overlapping_nodes, overlap_membership, num_nodes,
          member_matrix, excess, defect, degree_seq, num_seq,
          internal_degree_seq, fixed_range, nmin, nmax, tau2) == -1)
    return -1;

  deque<set<int>>
      E; // E is the adjacency matrix written in form of list of edges
  deque<deque<int>> member_list; // row i cointains the memberships of node i
  deque<deque<int>> link_list;   // row i cointains degree of the node i respect
                               // to member_list[i][j]; there is one more number
                               // that is the external degree

  cout << "building communities... " << endl;
  if (build_subgraphs(E, member_matrix, member_list, link_list,
                      internal_degree_seq, degree_seq, excess, defect) == -1)
    return -1;
  ...
  if (ca != unlikely) {
    cout << "trying to approach an average clustering coefficient ... " << ca
         << endl;
...
int main(int argc, char *argv[]) {
  ...
  benchmark(p.excess, p.defect, p.num_nodes, p.average_k, p.max_degree, p.tau,
            p.tau2, p.mixing_parameter, p.overlapping_nodes,
            p.overlap_membership, p.nmin, p.nmax, p.fixed_range,
            p.clustering_coeff);
