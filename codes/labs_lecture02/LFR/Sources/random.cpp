...
double ran4() { return ran4(true, 0); }
...
int irand(int n) { return (int(ran4() * (n + 1))); }
...
int configuration_model(deque<set<int>> &en, deque<int> &degrees) {
  ...
  // this function is to build a network with the degree seq in degrees which is
  // sorted (correspondence is based on the vectorial index)
  if (degrees.size() < 3) {
