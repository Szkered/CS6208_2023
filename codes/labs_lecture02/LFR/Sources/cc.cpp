...
int cclu(deque<set<int>> &en, const deque<deque<int>> &member_list,
  ...
  cout << "Average Clustering coefficient... " << cc0 << " trying to reach "
       << ca << endl;
  ...
  while (cc0 < ca) {
    ...
    for (int y = 0; y < num_p; y++)
      for (int Ai = 0; Ai < en.size(); Ai++) {
        ...
        while (true) {
          ...
          deque<int> not_ra;
          for (set<int>::iterator it_est = en[random_node].begin();
               it_est != en[random_node].end(); it_est++)
          ...
          deque<int> out_nodes;
          for (set<int>::iterator it_est = en[a].begin(); it_est != en[a].end();
               it_est++)
          ...
          // double c1=common_neighbors(a, old_node, en) +
          // common_neighbors(random_mate, node_h, en);
          double c1 = t1 + t2;
          ...
          double c2 = common_neighbors(a, random_mate, en) +
                      common_neighbors(old_node, node_h, en);
    ...
    if (cc0 - ccold < min_relative_inc * cc0) {
      ...
      cout << "It seems I cannot reach the wished value. I'll stop here..."
           << endl;
      break;
    }
    ...
    cout << "Average Clustering coefficient... " << cc0 << " trying to reach "
         << ca << "\t\t expected " << num_p << " more step(s) " << endl;
  }
