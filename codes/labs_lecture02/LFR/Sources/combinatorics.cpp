...
template <typename Seq> double average_func(Seq &sq) {
...
template <typename Seq> double variance_func(Seq &sq) {
...
template <typename Seq> double average_pf(Seq &sq) {
...
template <typename Seq> double variance_pf(Seq &sq) {
...
double binomial(int n, int x,
                double p) { //	returns the binomial distribution, n trials, x
...
int nn=lower_bound(cumulative.begin(), cumulative.end(),
ran4())-cumulative.begin();
...
// this function sets "cumulative" as the cumulative function of (1/x)^tau, with
// range= [min, n]
// to draw a number:
// int nn=lower_bound(cumulative.begin(), cumulative.end(),
// ran4())-cumulative.begin()+min_degree;
...
int distribution_from_cumulative(
    const deque<double> &cum,
    deque<double> &distr) { // cum is the cumulative, distr is set equal to the
                            // distribution
...
int cumulative_from_distribution(
    deque<double> &cum,
    const deque<double> &distr) { // cum is set equal to the cumulative, distr
                                  // is the distribution
...
int shuffle_and_set(
    int *due,
    int dim) { // it sets due as a random sequence of integers from 0 to dim-1
...
template <typename type_> int shuffle_s(type_ *a, int b) {
...
double compute_self_links(int k, int n, int x) {
        ...
        double prod= log_combination(n/2, k-x) + log_combination(k-x, x) +
(k-2*x) * log(2) - log_combination(n, k);
